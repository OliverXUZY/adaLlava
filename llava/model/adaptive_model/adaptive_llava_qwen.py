...
class AdaptiveLlavaQwenModel(LlavaMetaModel, AdaptiveQwen2Model):
...
class AdaptiveLlavaQwenForCausalLM(AdaptiveQwen2ForCausalLM, LlavaMetaForCausalLM):
    ...
    def __init__(self, config):
        ...
        AdaptiveQwen2ForCausalLM.__init__(self, config)
        config.model_type = "llava_qwen"
        config.rope_scaling = None

        self.model = AdaptiveLlavaQwenModel(config)
    ...
    def generate(
        ...
        **kwargs,
        ...
        # inputs_embeds: (bs, seq_length, dim)

        #TODO: Generate drop_mask for the token.
        # Assumption: For a sample, the execution branch is the same for all tokens.
        # 1. indicates keep, 0. indicates drop.
        # drop_mask can be generated in self.prepare_inputs_labels_for_multimodal method above or right below.
        # drop_mask: (num_hidden_layers, bs)
...
AutoModelForCausalLM.register(LlavaQwenConfig, AdaptiveLlavaQwenForCausalLM)
